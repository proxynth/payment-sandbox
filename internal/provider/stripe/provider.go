...
// Provider is a deterministic, stateless Stripe provider implementation. It
// adapts canonical provider requests without depending on Stripe SDK types.
...
func successfulResult(operation string, paymentID paymentdomain.ID) providerdomain.OperationResult {
	return providerdomain.OperationResult{
		...
		ProviderReference: fmt.Sprintf("stripe:%s:%s", operation, paymentID),
	}
}
