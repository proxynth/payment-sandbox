...
import (
	...
	deterministic "proxynth/payment-sandbox/internal/provider/deterministic"
...
// Invariant: the scenario seed reaches the configured provider and changes its
// deterministic outcome without introducing runtime randomness.
func TestAuditScenarioSeedDrivesProviderOutcome(t *testing.T) {
	registry := provider.NewRegistry()
	if err := registry.Register(deterministic.New("deterministic")); err != nil {
		t.Fatal(err)
	}
	runner := NewRunner(registry)
	for seed, want := range map[uint64]pd.Status{
		0: pd.StatusAuthorized,
		1: pd.StatusFailed,
		2: pd.StatusPending,
	} {
		paymentID := pd.ID("payment-" + fmt.Sprint(seed))
		scenario := validScenario(nil, []rd.Command{
			{Type: rd.CommandCreatePayment, PaymentID: paymentID, Amount: testMoney(t, 1000, "EUR")},
			{Type: rd.CommandAuthorize, PaymentID: paymentID},
		})
		scenario.Provider = rd.ProviderConfiguration{ID: "deterministic", Profile: "seeded"}
		scenario.DeterministicConfiguration.Seed = seed
		result, err := runner.Run(context.Background(), scenario)
		if err != nil {
			t.Fatalf("seed %d: Run() error = %v", seed, err)
		}
		if len(result.Payments) != 1 || result.Payments[0].Status != want {
			t.Fatalf("seed %d: payments = %+v, want status %s", seed, result.Payments, want)
		}
	}
}
