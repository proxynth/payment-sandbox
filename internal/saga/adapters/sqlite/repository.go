...
func (r *Repository) Save(ctx context.Context, instance domain.Instance) error {
	...
	_, err = r.db.ExecContext(ctx, `
		INSERT INTO saga_instances(id, payment_id, status, current_step, completed_steps, compensation_steps, seed, version, updated_at)
		...
		instance.ID, instance.PaymentID, instance.Status, instance.CurrentStep,
...
func (r *Repository) Find(ctx context.Context, id domain.ID) (domain.Instance, error) {
	...
	err := r.db.QueryRowContext(ctx, `SELECT id,payment_id,status,current_step,completed_steps,compensation_steps,seed,version,updated_at FROM saga_instances WHERE id=$1`, id).
		Scan(&instance.ID, &instance.PaymentID, &instance.Status, &instance.CurrentStep, &completed, &compensation, &instance.Seed, &instance.Version, &updated)
	if err != nil {
