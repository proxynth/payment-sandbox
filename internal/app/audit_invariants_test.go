...
// Invariant: reusing an idempotency key with a different command is rejected
// and cannot apply a second monetary effect.
func TestAuditHTTPIdempotencyFingerprintConflict(t *testing.T) {
	db := auditDB(t)
	a := auditRuntime(t, db)
	auditPayment(t, a)
	auditHTTP(t, a, "POST", "/payments/p/capture", `{"amount":400,"currency":"EUR"}`, "same-operation", 200)
	auditHTTP(t, a, "POST", "/payments/p/capture", `{"amount":300,"currency":"EUR"}`, "same-operation", 409)

	p, err := ps.NewRepository(db).FindByID(auditContext(), "p")
	if err != nil {
		t.Fatal(err)
	}
	if got := p.CapturedAmount().Amount(); got != 400 {
		t.Fatalf("fingerprint conflict changed captured amount = %d, want 400", got)
	}
}

// Invariant: concurrent identical requests with one idempotency key produce
// at most one monetary effect.
func TestAuditHTTPIdempotencyConcurrentRequests(t *testing.T) {
	db := auditDB(t)
	a := auditRuntime(t, db)
	auditPayment(t, a)
	...
	for range requests {
		go func() {
			defer group.Done()
			<-start
			statuses <- auditRequest(a, "POST", "/payments/p/capture", `{"amount":400,"currency":"EUR"}`, "concurrent-operation").Code
		}()
	}
	close(start)
	group.Wait()
	...
	for status := range statuses {
		if status != http.StatusOK && status != http.StatusConflict {
			t.Fatalf("concurrent idempotent request status = %d", status)
		}
	}
	p, err := ps.NewRepository(db).FindByID(auditContext(), "p")
	if err != nil {
	...
	if got := p.CapturedAmount().Amount(); got != 400 {
		t.Fatalf("concurrent idempotent requests changed captured amount = %d, want 400", got)
	}
}
...
// Invariant: a persisted scenario remains executable and deterministic after
// the application is recomposed from the same database.
func TestAuditScenarioExecutionSurvivesRestart(t *testing.T) {
	db := auditDB(t)
	first := auditRuntime(t, db)
	auditHTTP(t, first, "POST", "/admin/scenarios", `{"id":"restart-execution","provider":{"id":"fake"},"initial_virtual_time":"2026-01-01T00:00:00Z","commands":[{"type":"create_payment","payment_id":"scenario-p","amount":1000,"currency":"EUR"},{"type":"authorize","payment_id":"scenario-p"}]}`, "", 201)
	second := auditRuntime(t, db)
	firstResult := auditHTTP(t, second, "POST", "/admin/scenarios/restart-execution/execute", "", "", 200)
	secondResult := auditHTTP(t, second, "POST", "/admin/scenarios/restart-execution/execute", "", "", 200)
	if firstResult != secondResult {
		t.Fatal("persisted scenario execution was not deterministic after restart")
	}
	if auditCount(t, db, `SELECT count(*) FROM payments`) != 0 || auditCount(t, db, `SELECT count(*) FROM event_log`) != 0 {
		t.Fatal("persisted scenario execution modified runtime state")
	}
}

// Invariant: concurrent executions of one persisted scenario are isolated
// replay sessions and return the same deterministic result.
func TestAuditScenarioExecutionIsConcurrentAndIsolated(t *testing.T) {
	db := auditDB(t)
	a := auditRuntime(t, db)
	auditHTTP(t, a, "POST", "/admin/scenarios", `{"id":"concurrent-replay","provider":{"id":"fake"},"initial_virtual_time":"2026-01-01T00:00:00Z","commands":[{"type":"create_payment","payment_id":"scenario-p","amount":1000,"currency":"EUR"},{"type":"authorize","payment_id":"scenario-p"}]}`, "", 201)
	...
	var group sync.WaitGroup
	group.Add(executions)
	for range executions {
		go func() {
			defer group.Done()
			<-start
			response := auditRequest(a, "POST", "/admin/scenarios/concurrent-replay/execute", "", "")
			results <- executionResult{status: response.Code, body: response.Body.String()}
		}()
	}
	close(start)
	group.Wait()
	...
	for result := range results {
		if result.status != http.StatusOK {
			t.Fatalf("concurrent scenario execution status = %d, body=%s", result.status, result.body)
		}
		if baseline == "" {
		...
		if result.body != baseline {
			t.Fatal("concurrent scenario executions produced different results")
		}
	}
	if auditCount(t, db, `SELECT count(*) FROM payments`) != 0 || auditCount(t, db, `SELECT count(*) FROM event_log`) != 0 {
		t.Fatal("concurrent scenario executions modified runtime state")
	}
}
