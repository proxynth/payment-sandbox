...
func NewJobSnapshot(job *Job) JobSnapshot {
	return JobSnapshot{
		...
		LeaseExpiresAt: job.LeaseExpiresAt(), LeaseGeneration: job.LeaseGeneration(), Attempts: job.Attempts(), AggregateID: job.AggregateID(), CausationID: job.CausationID(),
	}
}

// Restore recreates the exact job state represented by the snapshot.
func (s JobSnapshot) Restore() (*Job, error) {
	job, err := Restore(s.ID, s.Type, s.Payload, s.ScheduledAt, s.NextAttemptAt, s.Status, s.LeaseOwner, s.LeaseExpiresAt, s.LeaseGeneration, s.Attempts, JobMetadata{AggregateID: s.AggregateID, CausationID: s.CausationID})
	if err != nil {
		return nil, err
	}
	return &job, nil
