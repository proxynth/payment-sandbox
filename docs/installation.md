...
Release tags use the `vMAJOR.MINOR.PATCH` format, such as `v0.1.0`.
...
`v0.1.0` Linux ARM64 archive is
`payment-sandbox_0.1.0_linux_arm64.tar.gz`.
...
```bash
sha256sum --check payment-sandbox_0.1.0_checksums.txt --ignore-missing
```

If `sha256sum` is not available on macOS, use:

```bash
shasum -a 256 payment-sandbox_0.1.0_linux_arm64.tar.gz
```
...
```powershell
Get-FileHash .\payment-sandbox_0.1.0_windows_amd64.zip -Algorithm SHA256
```

Compare the result with the matching entry in
`payment-sandbox_0.1.0_checksums.txt`.
...
```bash
mkdir payment-sandbox-0.1.0-linux-arm64
tar -xzf payment-sandbox_0.1.0_linux_arm64.tar.gz -C payment-sandbox-0.1.0-linux-arm64
cd payment-sandbox-0.1.0-linux-arm64
```
